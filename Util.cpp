...
std::string Encrypt(const std::string& inMessage, const std::string& key)
{
	Counter ctr;
	Feistel feistel;
	string* messageChiffre = new string[20];
	string messageCode;
	cout << "-- CRYPTAGE --" << endl;
	ctr.counterMode(inMessage, key);
	messageChiffre = ctr.getBlocsChiffres();

	cout << "-- Feistel --" << endl;
	for (int i = 0;i < ctr.nombreBlocs;i++)
	{
		messageCode.append(feistel.chiffrement(messageChiffre[i], key));
	}
	cout << "Message crypte: " << messageCode << endl;
	return messageCode;
}
...
std::string Decrypt(const std::string& inCryptogram, const std::string& key)
{
	...
	string* blocsDecodes = new string[20];
	string messageDecode;

	cout << "-- DECRYPTAGE --" << endl;
	ctr.decouperBlocs(inCryptogram);
	blocsChiffres = ctr.getDecoupageBlocs();

	cout << "-- Feistel --" << endl;
	cout << "Blocs decryptes: ";
	for (int i = 0;i < ctr.nombreBlocs;i++)
	{
		blocsDecodes[i] = feistel.dechiffrement(blocsChiffres[i], key);
		cout << blocsDecodes[i] << " ";
	}
	cout << endl;

	ctr.dechiffrerBlocs(blocsDecodes, key);
	messageDecode = ctr.getTexteDechiffre();
...
std::string CalculateMAC(const std::string& inMessage, const std::string& secret)
{
	...
	texteMac = mac.hmac(secret, inMessage);
